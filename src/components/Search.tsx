...
export function Search({ places, onToggle }: SearchProps): JSX.Element | null {
  ...
  return (
    <div className="Search">
      ...
      >
        <input name="query" type="text" placeholder="Rechercher un lieu" autoComplete="off" />
        <button type="submit">
          <MagnifyingGlass size={32} alt="Rechercher" role="img" />
        </button>
      </form>
      {data && (data.results === undefined || data.results?.length === 0) && <p>Aucun résultat</p>}
      <ul className="Search--results">
        {data &&
          data.results &&
          data.results.map((item) => {
            const active = places.find((place) => place.id === item.id) !== undefined;
            return (
              <PlaceItem
                key={item.id}
